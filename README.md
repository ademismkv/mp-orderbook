...
**Rust → `cxx` → C++ FFI overhead**: measured 5.7-12% slower than pure C++ depending on run/workload, p50 within 1-40ns of pure C++ — a real, measured language-boundary cost, not a guess (ADR-3).
...
- **10/10 unit tests** (v2), 5/5 (v1 reference), plus the zero-allocation hot-path check.
- **Differential fuzzing**: v1 vs. v2 checked over 1.5M+ operations across two workloads (fixed-price-band, and a price-random-walk generator added specifically to force the price-window rebase path), zero mismatches, run on every CI push.
...
- **Order types**: Limit only. No Market, IOC, FOK, or Post-Only — `Type::Market` exists in the core engine's type but isn't exercised by the ITCH pipeline above it.
