...
What's still unverified: a full `cargo build && cargo test` pass with both fixes applied, since this sandbox still has no `cargo`. That's the next real data point, not a hypothetical one — the loop (paste error, fix, re-run) is working as designed, and it's now found and fixed two genuine, independent bugs this way.
