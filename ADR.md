...
| Rust boundary | none | none | `cxx` bridge **implemented**, real `cargo build` run on the user's machine — one namespace bug found and fixed; FIX 4.4 inbound parser + async risk pre-check **implemented**, still owed a real `cargo test` run (no `cargo` in this sandbox) (ADR-3) |
