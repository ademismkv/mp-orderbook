...
**Update, day 10:** the `index_` map itself has since been replaced — `std::unordered_map<OrderId, IndexEntry>` swapped for a hand-written `FlatHashMap` (`cpp/include/flat_hash_map.h`: open addressing, backward-shift deletion, identity hashing — chosen specifically because real exchange-assigned order ids are sequential, and a scrambling hash was measured to hurt cache locality rather than help it). Re-verified against v1 by differential fuzzing (48 total runs across the original fixed-price-band workload and a new price-random-walk workload added specifically to force the price-window rebase path — see devlog day 10), plus TSan/ASan, plus the real NASDAQ replay (identical trade/violation/miss counts to every previous run). Measured effect, same sandbox, same `-O3` flag: p50 125ns → 84ns, p99 ~417-458ns → 375ns, throughput ~4.9-5.1M → ~5.8-6.0M ops/sec.

**Known cost, now measured instead of just asserted:** the price array rebases (grows/shifts) when an order's price falls outside the current window — O(n) in open orders/levels when it fires. `OrderBookV2::level_array_growths()` counts every time this path actually triggers. Measured: **0 growth events** over the 2,000,000-op synthetic benchmark (`bench/bench_v2.cpp`, prices confined to a ±50-tick range, well inside the default 20,000-tick window), and **1 growth event** over the full real 400,391-event AAPL trading day (`cpp/tools/replay_lobster.cpp`), which deliberately pre-sizes the window to 150,000 ticks — wide enough to cover that day's real $577.35–$588.32 range up front. So on the workloads this repo actually measures, "rare in practice" is now a real number, not just a comment. Two unit tests cover the mechanism itself (front-growth, back-growth), and repeated rebases in one run still haven't been fuzzed — worth doing if this book is ever run against a symbol with a genuinely wide, unpredictable intraday range instead of a pre-sizeable one.
...
|---|---|---|---|
| Book container | `std::map` + `std::deque` | array of price levels + intrusive list + arena + `FlatHashMap` id index (replaced `std::unordered_map`, day 10) | same — Node cache-padding tried, measured no single-threaded benefit, not shipped (devlog day 2) |
