...
// v1 side of the rebase-path differential test — see fuzz_v2_rebase.cpp and
// workload.h's generate_ops_price_walk for what this is checking and why.
// v1 (std::map-based, no price window) is otherwise identical to fuzz_v1.cpp.
int main(int argc, char** argv) {
    ...
    for (auto& op : ops) {
        if (op.kind == OpKind::Cancel) {
            book.add(Order{op.id, Side::Buy, Type::Cancel, 0.0, 0, 0});
            continue;
        }
        Order o;
        o.id = op.id;
        ...
        o.price = static_cast<double>(op.price);
        o.qty = op.qty;
        o.timestamp_ns = 0;
